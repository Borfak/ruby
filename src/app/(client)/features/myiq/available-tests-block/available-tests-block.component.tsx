  ...
  return (
    <Section background='default'>
      <Container variant='section'>
        <h3 className='mb-4 text-center text-4xl font-bold'>Available Tests</h3>
          ...
          {TESTS.map((test, index) => (
            <Card key={index} padding={false} className='flex flex-col items-start p-4 text-left'>
              ...
              <Button
                ...
                className={`w-full text-sm ${test.disabled ? 'cursor-not-allowed bg-teal-600 text-white opacity-70' : ''}`}
              >
                {test.buttonText}
              </Button>
            </Card>
