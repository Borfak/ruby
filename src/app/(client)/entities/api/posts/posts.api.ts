...
export const postsQueryApi = {
  ...
  byId: (opt: QueryFunctionContext, { id }: { id: number }) => apiGet<Post>(opt, `posts/${id}`),
  ...
  userById: (opt: QueryFunctionContext, { userId }: { userId: number }) => apiGet<User>(opt, `users/${userId}`),
}
